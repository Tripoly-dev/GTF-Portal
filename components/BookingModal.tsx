  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(7,26,23,0.6)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, backdropFilter: 'blur(4px)' }}>
      <div style={{ background: 'white', width: '100%', maxWidth: 640, maxHeight: '92vh', overflowY: 'auto', borderRadius: 16, boxShadow: '0 32px 80px rgba(7,26,23,0.25)' }}>
        ...
        {/* Error */}
        {error && (
          <div style={{ margin: '16px 28px 0', padding: '12px 16px', background: 'var(--danger-bg)', border: '1px solid var(--danger-bg)', borderRadius: 8, fontSize: 13, color: 'var(--danger)' }}>{error}</div>
        )}
        ...
        <div style={{ padding: '24px 28px 28px' }}>
                    ...
                    <div style={{ gridColumn: '1 / -1' }}>
                      ...
                      {p.passport_url ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--ok-bg)', borderRadius: 6, border: '1px solid #6ee7b7' }}>
                          ...
                          <span style={{ fontSize: 12, color: 'var(--ok)', fontWeight: 600 }}>Passport uploaded successfully</span>
                          <button onClick={() => updatePassenger(idx, 'passport_url', '')} style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: 'var(--ok)', textDecoration: 'underline' }}>Remove</button>
                        </div>
                      ) : (
                        <div>
          ...
          {step === 3 && (
            <div>
                ...
                {passengers.map((p, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--rule)' }}>
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--ink-light)' }}>{paxLabel(i)} · Passport: {p.passport_number}</div>
                    </div>
                    <div style={{ textAlign: 'right', fontSize: 12, color: 'var(--ink-light)' }}>
                      <div>DOI: {p.doi}</div>
                      <div>DOE: {p.doe}</div>
                      {p.passport_url && <div style={{ color: 'var(--ok)', fontWeight: 600 }}>✓ Passport uploaded</div>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ marginTop: 20, padding: '14px 16px', background: 'var(--warn-lt)', borderRadius: 8, border: '1px solid var(--warn-lt)', fontSize: 12, color: 'var(--warn)' }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 28 }}>
            {step > 1 && (
              <button onClick={() => setStep(s => s - 1)} style={{ flex: 1, padding: '13px', background: 'none', border: '1.5px solid var(--rule)', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600, color: 'var(--ink-mid)', fontFamily: 'var(--font-sans)' }}>
                ← Back
              </button>
            )}
            {step < STEPS.length ? (
