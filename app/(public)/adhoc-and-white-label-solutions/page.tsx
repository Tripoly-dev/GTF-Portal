    ...
    <div style={{ background: 'var(--bg)' }}>
      ...
      <div style={{ position: 'relative', height: 440, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '52px 56px' }}>
          ...
          <p className="font-tight" style={{ fontSize: 18, fontWeight: 400, fontStyle: 'italic', color: 'rgba(255,255,255,0.65)', maxWidth: 560 }}>
          ...
          {[
            ...
            <div key={s.label} style={{ flex: 1, paddingLeft: i > 0 ? 40 : 0, borderLeft: i > 0 ? '1px solid var(--rule)' : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                ...
                <span className="font-tight" style={{ fontSize: 28, fontWeight: 400, color: 'var(--teal)', lineHeight: 1 }}>{s.sup}</span>
              </div>
              <div style={{ fontSize: 12, letterSpacing: '0.04em', color: 'var(--ink-light)', marginTop: 6, fontWeight: 600, textTransform: 'uppercase' }}>{s.label}</div>
            </div>
          ))}
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0 }}>
            {SERVICES.map((s, i) => (
              <div key={s} style={{
                ...
                <span style={{ fontSize: 15, fontWeight: 500, color: 'var(--ink-mid)' }}>{s}</span>
              </div>
            ))}
          </div>
        </div>
