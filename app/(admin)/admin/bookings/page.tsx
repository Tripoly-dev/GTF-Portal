                      ...
                      <td style={{ padding: '14px 16px' }}>
                        {b.status === 'pending' && (
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button onClick={() => updateStatus(b.id, 'confirmed')} disabled={isUpdating} style={{ padding: '6px 12px', background: 'var(--ok)', color: 'white', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer', opacity: isUpdating ? 0.5 : 1 }}>
                              {isUpdating ? '...' : 'Confirm'}
                            </button>
                            <button onClick={() => updateStatus(b.id, 'cancelled')} disabled={isUpdating} style={{ padding: '6px 12px', background: 'var(--danger)', color: 'white', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer', opacity: isUpdating ? 0.5 : 1 }}>
                              Cancel
                            </button>
                          </div>
                        )}
